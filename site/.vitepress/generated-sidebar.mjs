export const generatedSidebar = {
  "/agents/README": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "仓库说明与使用建议",
      "link": "/agents/README"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "文件说明",
          "link": "/agents/README#文件说明"
        },
        {
          "text": "使用建议",
          "link": "/agents/README#使用建议"
        },
        {
          "text": "维护规则",
          "link": "/agents/README#维护规则"
        },
        {
          "text": "更新记录",
          "link": "/agents/README#更新记录"
        }
      ]
    }
  ],
  "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "【复用提示词】上市公司重大资产重组竞聘——案例检索报告制作",
      "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、变量替换区（每次项目替换这里）",
          "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#一、变量替换区-每次项目替换这里"
        },
        {
          "text": "二、核心指令（整合版，整段发送给AI）",
          "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#二、核心指令-整合版-整段发送给ai"
        },
        {
          "text": "三、适配其他项目类型的调整要点",
          "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#三、适配其他项目类型的调整要点"
        },
        {
          "text": "四、关键经验沉淀（避坑指南）",
          "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#四、关键经验沉淀-避坑指南",
          "items": [
            {
              "text": "检索阶段",
              "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#检索阶段"
            },
            {
              "text": "内容制作阶段",
              "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#内容制作阶段"
            },
            {
              "text": "述标准备阶段",
              "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#述标准备阶段"
            }
          ]
        },
        {
          "text": "五、质量检查清单（报告完成前逐项核对）",
          "link": "/agents/【复用】上市公司重大资产重组竞聘-案例检索报告-提示词模板#五、质量检查清单-报告完成前逐项核对"
        }
      ]
    }
  ],
  "/agents/合同审查和起草": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "合同审查和起草指令",
      "link": "/agents/合同审查和起草"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "角色定位与默认立场",
          "link": "/agents/合同审查和起草#角色定位与默认立场"
        },
        {
          "text": "一、合同审查工作流",
          "link": "/agents/合同审查和起草#一、合同审查工作流",
          "items": [
            {
              "text": "（一）先概述协议主要内容",
              "link": "/agents/合同审查和起草#一-先概述协议主要内容"
            },
            {
              "text": "（二）再按风险等级进行审查",
              "link": "/agents/合同审查和起草#二-再按风险等级进行审查"
            },
            {
              "text": "（三）补充缺失条款及完善建议",
              "link": "/agents/合同审查和起草#三-补充缺失条款及完善建议"
            },
            {
              "text": "（四）同步进行文字、逻辑、格式及序号校对",
              "link": "/agents/合同审查和起草#四-同步进行文字、逻辑、格式及序号校对"
            },
            {
              "text": "（五）最后提供给客户的简短汇报",
              "link": "/agents/合同审查和起草#五-最后提供给客户的简短汇报"
            },
            {
              "text": "（六）文档修订模式",
              "link": "/agents/合同审查和起草#六-文档修订模式"
            }
          ]
        },
        {
          "text": "二、法律文件起草要求",
          "link": "/agents/合同审查和起草#二、法律文件起草要求"
        }
      ]
    }
  ],
  "/agents/法律PPT设计": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "法律 PPT 设计指令",
      "link": "/agents/法律PPT设计"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、适用场景",
          "link": "/agents/法律PPT设计#一、适用场景"
        },
        {
          "text": "二、整体视觉风格",
          "link": "/agents/法律PPT设计#二、整体视觉风格"
        },
        {
          "text": "三、页面结构与版式",
          "link": "/agents/法律PPT设计#三、页面结构与版式"
        },
        {
          "text": "四、字体与文字排版",
          "link": "/agents/法律PPT设计#四、字体与文字排版"
        },
        {
          "text": "五、信息呈现方式",
          "link": "/agents/法律PPT设计#五、信息呈现方式"
        },
        {
          "text": "六、视觉元素使用",
          "link": "/agents/法律PPT设计#六、视觉元素使用"
        },
        {
          "text": "七、修改已有 PPT 的要求",
          "link": "/agents/法律PPT设计#七、修改已有-ppt-的要求"
        },
        {
          "text": "八、新建 PPT 的执行流程",
          "link": "/agents/法律PPT设计#八、新建-ppt-的执行流程"
        }
      ]
    }
  ],
  "/agents/法律业务通用指令": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "法律业务通用指令",
      "link": "/agents/法律业务通用指令"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、角色定位与默认立场",
          "link": "/agents/法律业务通用指令#一、角色定位与默认立场"
        },
        {
          "text": "二、保密与信息安全",
          "link": "/agents/法律业务通用指令#二、保密与信息安全"
        },
        {
          "text": "三、语言、法域与法律依据核验",
          "link": "/agents/法律业务通用指令#三、语言、法域与法律依据核验"
        },
        {
          "text": "四、反编造红线",
          "link": "/agents/法律业务通用指令#四、反编造红线"
        },
        {
          "text": "五、执行流程",
          "link": "/agents/法律业务通用指令#五、执行流程"
        },
        {
          "text": "六、文档修订模式",
          "link": "/agents/法律业务通用指令#六、文档修订模式"
        },
        {
          "text": "七、起草默认值",
          "link": "/agents/法律业务通用指令#七、起草默认值"
        },
        {
          "text": "八、Word 文档格式要求",
          "link": "/agents/法律业务通用指令#八、word-文档格式要求",
          "items": [
            {
              "text": "（一）正文与标题",
              "link": "/agents/法律业务通用指令#一-正文与标题"
            },
            {
              "text": "（二）表格",
              "link": "/agents/法律业务通用指令#二-表格"
            },
            {
              "text": "（三）其他格式",
              "link": "/agents/法律业务通用指令#三-其他格式"
            }
          ]
        },
        {
          "text": "九、律所名称与署名",
          "link": "/agents/法律业务通用指令#九、律所名称与署名"
        },
        {
          "text": "十、法律分析与引证",
          "link": "/agents/法律业务通用指令#十、法律分析与引证"
        },
        {
          "text": "十一、跨境与外国法事项",
          "link": "/agents/法律业务通用指令#十一、跨境与外国法事项"
        },
        {
          "text": "十二、专业语气要求",
          "link": "/agents/法律业务通用指令#十二、专业语气要求"
        },
        {
          "text": "十三、交付方式",
          "link": "/agents/法律业务通用指令#十三、交付方式"
        }
      ]
    }
  ],
  "/agents/法律尽职调查清单起草与定稿": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "起草并定稿法律尽职调查清单的指令",
      "link": "/agents/法律尽职调查清单起草与定稿"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、项目资料",
          "link": "/agents/法律尽职调查清单起草与定稿#一、项目资料"
        },
        {
          "text": "二、总体起草要求",
          "link": "/agents/法律尽职调查清单起草与定稿#二、总体起草要求"
        },
        {
          "text": "三、清单完善要求",
          "link": "/agents/法律尽职调查清单起草与定稿#三、清单完善要求"
        },
        {
          "text": "四、自然人调查表要求",
          "link": "/agents/法律尽职调查清单起草与定稿#四、自然人调查表要求"
        },
        {
          "text": "五、用户修改优先原则",
          "link": "/agents/法律尽职调查清单起草与定稿#五、用户修改优先原则"
        },
        {
          "text": "六、文件格式及命名",
          "link": "/agents/法律尽职调查清单起草与定稿#六、文件格式及命名"
        },
        {
          "text": "七、验收要求",
          "link": "/agents/法律尽职调查清单起草与定稿#七、验收要求"
        }
      ]
    }
  ],
  "/agents/法律尽调报告提示词": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "法律尽调报告提示词",
      "link": "/agents/法律尽调报告提示词"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、使用方式",
          "link": "/agents/法律尽调报告提示词#一、使用方式"
        },
        {
          "text": "二、股权历史沿革部分",
          "link": "/agents/法律尽调报告提示词#二、股权历史沿革部分",
          "items": [
            {
              "text": "（一）任务目标",
              "link": "/agents/法律尽调报告提示词#一-任务目标"
            },
            {
              "text": "（二）输入材料",
              "link": "/agents/法律尽调报告提示词#二-输入材料"
            },
            {
              "text": "（三）核查与梳理要求",
              "link": "/agents/法律尽调报告提示词#三-核查与梳理要求"
            },
            {
              "text": "（四）底稿 PDF 整理要求",
              "link": "/agents/法律尽调报告提示词#四-底稿-pdf-整理要求"
            },
            {
              "text": "（五）报告正文写作要求",
              "link": "/agents/法律尽调报告提示词#五-报告正文写作要求"
            },
            {
              "text": "（六）历史沿革补充材料清单",
              "link": "/agents/法律尽调报告提示词#六-历史沿革补充材料清单"
            },
            {
              "text": "（七）输出成果与验收要求",
              "link": "/agents/法律尽调报告提示词#七-输出成果与验收要求"
            }
          ]
        },
        {
          "text": "三、后续章节扩展格式",
          "link": "/agents/法律尽调报告提示词#三、后续章节扩展格式"
        }
      ]
    }
  ],
  "/agents/股改文件起草": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "股改文件起草指令",
      "link": "/agents/股改文件起草"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、项目输入",
          "link": "/agents/股改文件起草#一、项目输入"
        },
        {
          "text": "二、工作要求",
          "link": "/agents/股改文件起草#二、工作要求"
        },
        {
          "text": "三、股改文件范围",
          "link": "/agents/股改文件起草#三、股改文件范围"
        },
        {
          "text": "四、特殊治理结构适配",
          "link": "/agents/股改文件起草#四、特殊治理结构适配"
        },
        {
          "text": "五、重点一致性检查",
          "link": "/agents/股改文件起草#五、重点一致性检查"
        },
        {
          "text": "六、模板残留清理",
          "link": "/agents/股改文件起草#六、模板残留清理"
        },
        {
          "text": "七、格式要求",
          "link": "/agents/股改文件起草#七、格式要求"
        },
        {
          "text": "八、校验要求",
          "link": "/agents/股改文件起草#八、校验要求"
        },
        {
          "text": "九、缺失信息处理",
          "link": "/agents/股改文件起草#九、缺失信息处理"
        },
        {
          "text": "十、最终交付",
          "link": "/agents/股改文件起草#十、最终交付"
        }
      ]
    }
  ],
  "/agents/非诉业务常用提示词": [
    {
      "text": "返回栏目列表",
      "link": "/agents/"
    },
    {
      "text": "非诉业务常用提示词",
      "link": "/agents/非诉业务常用提示词"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、尽调底稿三级目录制作提示词",
          "link": "/agents/非诉业务常用提示词#一、尽调底稿三级目录制作提示词"
        },
        {
          "text": "二、尽调底稿二级目录生成提示词",
          "link": "/agents/非诉业务常用提示词#二、尽调底稿二级目录生成提示词",
          "items": [
            {
              "text": "使用方式",
              "link": "/agents/非诉业务常用提示词#使用方式"
            },
            {
              "text": "提示词正文",
              "link": "/agents/非诉业务常用提示词#提示词正文"
            }
          ]
        }
      ]
    }
  ],
  "/agents/": [
    {
      "text": "智能体通用指令和项目指令",
      "link": "/agents/"
    }
  ],
  "/skills/cnipa-patent-evidence-archive": [
    {
      "text": "返回栏目列表",
      "link": "/skills/"
    },
    {
      "text": "CNIPA 专利证据归档",
      "link": "/skills/cnipa-patent-evidence-archive"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "目标",
          "link": "/skills/cnipa-patent-evidence-archive#goal"
        },
        {
          "text": "输入信息",
          "link": "/skills/cnipa-patent-evidence-archive#inputs"
        },
        {
          "text": "工作流程",
          "link": "/skills/cnipa-patent-evidence-archive#workflow"
        },
        {
          "text": "实务注意事项",
          "link": "/skills/cnipa-patent-evidence-archive#practical-notes"
        }
      ]
    }
  ],
  "/skills/cnipa-trademark-evidence-archive": [
    {
      "text": "返回栏目列表",
      "link": "/skills/"
    },
    {
      "text": "CNIPA 商标证据归档",
      "link": "/skills/cnipa-trademark-evidence-archive"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "目标",
          "link": "/skills/cnipa-trademark-evidence-archive#goal"
        },
        {
          "text": "输入信息",
          "link": "/skills/cnipa-trademark-evidence-archive#inputs"
        },
        {
          "text": "工作流程",
          "link": "/skills/cnipa-trademark-evidence-archive#workflow"
        },
        {
          "text": "Chrome 自动化路径",
          "link": "/skills/cnipa-trademark-evidence-archive#scripted-chrome-path"
        },
        {
          "text": "实务注意事项",
          "link": "/skills/cnipa-trademark-evidence-archive#practical-notes"
        },
        {
          "text": "失败运行经验",
          "link": "/skills/cnipa-trademark-evidence-archive#lessons-from-failed-runs"
        }
      ]
    }
  ],
  "/skills/network-check-v3": [
    {
      "text": "返回栏目列表",
      "link": "/skills/"
    },
    {
      "text": "中国企业网络核查",
      "link": "/skills/network-check-v3"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "适用场景",
          "link": "/skills/network-check-v3#when-to-use"
        },
        {
          "text": "安装与准备",
          "link": "/skills/network-check-v3#setup"
        },
        {
          "text": "主要命令",
          "link": "/skills/network-check-v3#main-command"
        },
        {
          "text": "支持的平台",
          "link": "/skills/network-check-v3#supported-platforms"
        },
        {
          "text": "推荐法律工作流程",
          "link": "/skills/network-check-v3#recommended-legal-workflow"
        },
        {
          "text": "平台注意事项",
          "link": "/skills/network-check-v3#platform-notes"
        },
        {
          "text": "安全与证据处理",
          "link": "/skills/network-check-v3#safety-and-evidence-handling"
        }
      ]
    }
  ],
  "/skills/": [
    {
      "text": "法律业务 Skill 技能",
      "link": "/skills/"
    }
  ],
  "/tutorials/agent-instruction-tool-selection-training-ABL-20260708-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "智能体指令体系、工具选择培训讲义（HTML 翻页版）",
      "link": "/tutorials/agent-instruction-tool-selection-training-ABL-20260708-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "内容索引",
          "link": "/tutorials/agent-instruction-tool-selection-training-ABL-20260708-V1#内容索引"
        }
      ]
    }
  ],
  "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "Claude App 与 Codex App 通过 CC Switch 接入国产大模型教程",
      "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "1. 工具与下载链接",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_1-工具与下载链接"
        },
        {
          "text": "2. 原理：为什么 Claude App 和 Codex App 的配置方式不一样",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_2-原理-为什么-claude-app-和-codex-app-的配置方式不一样"
        },
        {
          "text": "3. 安装准备",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_3-安装准备",
          "items": [
            {
              "text": "3.1 macOS",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_3-1-macos"
            },
            {
              "text": "3.2 Windows",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_3-2-windows"
            }
          ]
        },
        {
          "text": "4. CC Switch 基本界面",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_4-cc-switch-基本界面"
        },
        {
          "text": "5. Claude App 接入 DeepSeek",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_5-claude-app-接入-deepseek",
          "items": [
            {
              "text": "5.1 操作步骤",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_5-1-操作步骤"
            },
            {
              "text": "5.2 判断是否接入成功",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_5-2-判断是否接入成功"
            },
            {
              "text": "5.3 常见问题",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_5-3-常见问题"
            }
          ]
        },
        {
          "text": "6. Claude App 接入智谱 GLM Coding Plan / 智谱 API",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_6-claude-app-接入智谱-glm-coding-plan-智谱-api",
          "items": [
            {
              "text": "6.1 中国大陆站 GLM Coding Plan",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_6-1-中国大陆站-glm-coding-plan"
            },
            {
              "text": "6.2 Z.AI 国际站 GLM Coding Plan",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_6-2-z-ai-国际站-glm-coding-plan"
            },
            {
              "text": "6.3 操作步骤",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_6-3-操作步骤"
            }
          ]
        },
        {
          "text": "7. Codex App 接入 DeepSeek",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_7-codex-app-接入-deepseek",
          "items": [
            {
              "text": "7.1 第一次启动 Codex App",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_7-1-第一次启动-codex-app"
            },
            {
              "text": "7.2 在 CC Switch 添加 DeepSeek Codex 供应商",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_7-2-在-cc-switch-添加-deepseek-codex-供应商"
            },
            {
              "text": "7.3 开启 Local Routing",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_7-3-开启-local-routing"
            },
            {
              "text": "7.4 验证",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_7-4-验证"
            }
          ]
        },
        {
          "text": "8. Codex App 接入智谱 GLM Coding Plan / 智谱 API",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_8-codex-app-接入智谱-glm-coding-plan-智谱-api",
          "items": [
            {
              "text": "8.1 中国大陆站",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_8-1-中国大陆站"
            },
            {
              "text": "8.2 Z.AI 国际站",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_8-2-z-ai-国际站"
            },
            {
              "text": "8.3 操作步骤",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_8-3-操作步骤"
            }
          ]
        },
        {
          "text": "9. macOS 与 Windows 的差异",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_9-macos-与-windows-的差异"
        },
        {
          "text": "10. 常见报错与处理",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-常见报错与处理",
          "items": [
            {
              "text": "10.1 Codex 报 404：找不到 /responses",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-1-codex-报-404-找不到-responses"
            },
            {
              "text": "10.2 DeepSeek 上游报 404",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-2-deepseek-上游报-404"
            },
            {
              "text": "10.3 Codex App 看不到自定义模型",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-3-codex-app-看不到自定义模型"
            },
            {
              "text": "10.4 Claude App 重启后仍没有变化",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-4-claude-app-重启后仍没有变化"
            },
            {
              "text": "10.5 401 / Unauthorized",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_10-5-401-unauthorized"
            }
          ]
        },
        {
          "text": "11. 推荐配置选择",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_11-推荐配置选择",
          "items": [
            {
              "text": "11.1 Claude App",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_11-1-claude-app"
            },
            {
              "text": "11.2 Codex App",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_11-2-codex-app"
            }
          ]
        },
        {
          "text": "12. 安全与合规提示",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_12-安全与合规提示"
        },
        {
          "text": "13. 一页版流程",
          "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#_13-一页版流程",
          "items": [
            {
              "text": "Claude App + DeepSeek / 智谱",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#claude-app-deepseek-智谱"
            },
            {
              "text": "Codex App + DeepSeek / 智谱",
              "link": "/tutorials/claude-codex-app-ccswitch-domestic-models-ABL-20260707-V1#codex-app-deepseek-智谱"
            }
          ]
        }
      ]
    }
  ],
  "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "提示词：竞聘法律顾问·公司基本情况及法律问题初步分析报告",
      "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、任务目标",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#一、任务目标"
        },
        {
          "text": "二、输入信息（按项目替换）",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#二、输入信息-按项目替换"
        },
        {
          "text": "三、工作流程（AI 应严格按此顺序执行）",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#三、工作流程-ai-应严格按此顺序执行",
          "items": [
            {
              "text": "步骤 1：阅读内部资料",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#步骤-1-阅读内部资料"
            },
            {
              "text": "步骤 2：检索公开信息（与步骤1并行）",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#步骤-2-检索公开信息-与步骤1并行"
            },
            {
              "text": "步骤 3：梳理法律问题",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#步骤-3-梳理法律问题"
            },
            {
              "text": "步骤 4：生成 Word 文档",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#步骤-4-生成-word-文档"
            }
          ]
        },
        {
          "text": "四、关键质量要点（展示专业能力的地方）",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#四、关键质量要点-展示专业能力的地方"
        },
        {
          "text": "五、常见坑与避坑指南（踩过的坑）",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#五、常见坑与避坑指南-踩过的坑"
        },
        {
          "text": "六、Word 文档格式规范（律所默认）",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#六、word-文档格式规范-律所默认",
          "items": [
            {
              "text": "正文与标题",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#正文与标题"
            },
            {
              "text": "表格",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#表格"
            },
            {
              "text": "其他",
              "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#其他"
            }
          ]
        },
        {
          "text": "七、可直接发送给 AI 的提示词模板",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#七、可直接发送给-ai-的提示词模板"
        },
        {
          "text": "八、文件命名与版本管理",
          "link": "/tutorials/ipo-legal-advisor-company-profile-prompt-ABL-20260713-V1#八、文件命名与版本管理"
        }
      ]
    }
  ],
  "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "法律工作如何选择AI智能体（2026年7月）",
      "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、不同智能体的具体使用建议",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#一、不同智能体的具体使用建议"
        },
        {
          "text": "二、按照法律业务分类的智能体选择表",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#二、按照法律业务分类的智能体选择表"
        },
        {
          "text": "三、不要按产品选AI，要按任务选AI",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#三、不要按产品选ai-要按任务选ai",
          "items": [
            {
              "text": "（一）简明分工",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#一-简明分工"
            },
            {
              "text": "（二）选择公式",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#二-选择公式"
            },
            {
              "text": "（三）中国大陆法律用户的可用性与文档写作替代方案",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#三-中国大陆法律用户的可用性与文档写作替代方案"
            }
          ]
        },
        {
          "text": "四、诉讼业务实操：不同环节用不同智能体",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#四、诉讼业务实操-不同环节用不同智能体",
          "items": [
            {
              "text": "（一）案情初步梳理",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#一-案情初步梳理"
            },
            {
              "text": "（二）证据目录制作",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#二-证据目录制作"
            },
            {
              "text": "（三）起诉状、答辩状、仲裁申请书",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#三-起诉状、答辩状、仲裁申请书"
            },
            {
              "text": "（四）质证意见",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#四-质证意见"
            },
            {
              "text": "（五）庭审准备和攻防推演",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#五-庭审准备和攻防推演"
            }
          ]
        },
        {
          "text": "五、非诉业务实操：AI适合做底稿、清单、初稿和风险归纳",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#五、非诉业务实操-ai适合做底稿、清单、初稿和风险归纳",
          "items": [
            {
              "text": "（一）文档写作与报告起草",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#一-文档写作与报告起草"
            },
            {
              "text": "（二）合同审查",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#二-合同审查"
            },
            {
              "text": "（三）法律尽调",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#三-法律尽调"
            },
            {
              "text": "（四）法律检索和法律备忘录",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#四-法律检索和法律备忘录"
            },
            {
              "text": "（五）投融资、并购、股改、基金专项",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#五-投融资、并购、股改、基金专项"
            }
          ]
        },
        {
          "text": "六、行政类法律相关业务实操：优先交给办公智能体和编码智能体",
          "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#六、行政类法律相关业务实操-优先交给办公智能体和编码智能体",
          "items": [
            {
              "text": "（一）发票整理",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#一-发票整理"
            },
            {
              "text": "（二）投标文件制作",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#二-投标文件制作"
            },
            {
              "text": "（三）文件归档和项目底稿",
              "link": "/tutorials/legal-ai-agent-selection-202607-ABL-20260708-V1#三-文件归档和项目底稿"
            }
          ]
        }
      ]
    }
  ],
  "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "法律业务 Skill 制作教程：两种主要方案",
      "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、什么是法律业务 Skill",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#一、什么是法律业务-skill"
        },
        {
          "text": "二、方案一：完整运行一次业务流程，再由 AI 总结生成 Skill",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#二、方案一-完整运行一次业务流程-再由-ai-总结生成-skill",
          "items": [
            {
              "text": "（一）适用情况",
              "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#一-适用情况"
            },
            {
              "text": "（二）操作步骤",
              "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#二-操作步骤"
            }
          ]
        },
        {
          "text": "三、方案二：使用 Record & Replay 录制工作流，再生成 Skill",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#三、方案二-使用-record-replay-录制工作流-再生成-skill",
          "items": [
            {
              "text": "（一）适用情况",
              "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#一-适用情况-1"
            },
            {
              "text": "（二）操作步骤",
              "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#二-操作步骤-1"
            }
          ]
        },
        {
          "text": "四、两种制作方案对比",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#四、两种制作方案对比"
        },
        {
          "text": "五、并非所有法律工作都适合制作 Skill",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#五、并非所有法律工作都适合制作-skill"
        },
        {
          "text": "六、法律业务 Skill 的基本质量标准",
          "link": "/tutorials/legal-business-skill-creation-guide-ABL-20260710-V1#六、法律业务-skill-的基本质量标准"
        }
      ]
    }
  ],
  "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "macOS + Codex 法律工作流环境安装教程",
      "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "1. 任务理解与安装策略",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_1-任务理解与安装策略"
        },
        {
          "text": "2. 先安装 Apple 命令行工具",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_2-先安装-apple-命令行工具"
        },
        {
          "text": "3. 安装 Homebrew",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_3-安装-homebrew"
        },
        {
          "text": "4. 安装基础命令行工具",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_4-安装基础命令行工具"
        },
        {
          "text": "5. 安装浏览器与办公组件",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_5-安装浏览器与办公组件"
        },
        {
          "text": "6. 安装 PDF、图片与 OCR 工具",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_6-安装-pdf、图片与-ocr-工具"
        },
        {
          "text": "7. 建立 Python 法律工作虚拟环境",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_7-建立-python-法律工作虚拟环境"
        },
        {
          "text": "8. 法律文档处理的推荐验证链条",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_8-法律文档处理的推荐验证链条",
          "items": [
            {
              "text": "8.1 DOCX 合同审查",
              "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_8-1-docx-合同审查"
            },
            {
              "text": "8.2 PDF 底稿验证",
              "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_8-2-pdf-底稿验证"
            }
          ]
        },
        {
          "text": "9. Codex 全局法律工作配置",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_9-codex-全局法律工作配置",
          "items": [
            {
              "text": "9.1 创建全局 AGENTS.md",
              "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_9-1-创建全局-agents-md"
            },
            {
              "text": "9.2 项目级 AGENTS.md",
              "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_9-2-项目级-agents-md"
            }
          ]
        },
        {
          "text": "10. 建议安装或保留的法律工作 Skills",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_10-建议安装或保留的法律工作-skills"
        },
        {
          "text": "11. MCP 与外部数据源",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_11-mcp-与外部数据源"
        },
        {
          "text": "12. 浏览器证据 PDF 规则",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_12-浏览器证据-pdf-规则"
        },
        {
          "text": "13. 一键安装脚本",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_13-一键安装脚本"
        },
        {
          "text": "14. 完整验收清单",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_14-完整验收清单"
        },
        {
          "text": "15. 推荐目录结构",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_15-推荐目录结构"
        },
        {
          "text": "16. 什么时候需要额外安装",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_16-什么时候需要额外安装"
        },
        {
          "text": "17. 官方参考来源",
          "link": "/tutorials/macos-codex-legal-workflow-setup-ABL-20260707-V1#_17-官方参考来源"
        }
      ]
    }
  ],
  "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "macOS Codex 跳过登录页面教程（使用第三方模型）",
      "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、适用场景",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#一、适用场景"
        },
        {
          "text": "二、问题原因",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#二、问题原因"
        },
        {
          "text": "三、操作步骤",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#三、操作步骤",
          "items": [
            {
              "text": "步骤 1：完全退出 Codex",
              "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#步骤-1-完全退出-codex"
            },
            {
              "text": "步骤 2：备份现有配置（强烈建议）",
              "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#步骤-2-备份现有配置-强烈建议"
            },
            {
              "text": "步骤 3：修改 config.toml",
              "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#步骤-3-修改-config-toml"
            },
            {
              "text": "步骤 4：重置 auth.json",
              "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#步骤-4-重置-auth-json"
            },
            {
              "text": "步骤 5：重新启动 Codex",
              "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#步骤-5-重新启动-codex"
            }
          ]
        },
        {
          "text": "四、验证是否生效",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#四、验证是否生效"
        },
        {
          "text": "五、常见问题排查",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#五、常见问题排查"
        },
        {
          "text": "六、一键脚本（可选）",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#六、一键脚本-可选"
        },
        {
          "text": "七、注意事项",
          "link": "/tutorials/macos-codex-skip-login-third-party-models-ABL-20260708-V1#七、注意事项"
        }
      ]
    }
  ],
  "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "新建 VPS 后 OpenCode 安装与远程 Web UI 配置教程",
      "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "一、通过服务商控制台进入 VPS",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#一、通过服务商控制台进入-vps"
        },
        {
          "text": "二、准备 SSH Key",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#二、准备-ssh-key"
        },
        {
          "text": "三、判断 SSH 报错阶段",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#三、判断-ssh-报错阶段"
        },
        {
          "text": "四、在 VPS 内修复 SSH 服务",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#四、在-vps-内修复-ssh-服务"
        },
        {
          "text": "五、安装 OpenCode",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#五、安装-opencode"
        },
        {
          "text": "六、前台启动 OpenCode Web UI",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#六、前台启动-opencode-web-ui"
        },
        {
          "text": "七、安全与常驻运行建议",
          "link": "/tutorials/new-vps-opencode-web-ui-setup-ABL-20260709-V1#七、安全与常驻运行建议"
        }
      ]
    }
  ],
  "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "代理工具全平台使用教程",
      "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "1. 使用边界与重要说明",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_1-使用边界与重要说明"
        },
        {
          "text": "2. 先理解几个概念",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_2-先理解几个概念"
        },
        {
          "text": "3. 软件选择总览",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_3-软件选择总览"
        },
        {
          "text": "4. 准备工作",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_4-准备工作"
        },
        {
          "text": "5. macOS：Clash Verge Rev 完整步骤",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-macos-clash-verge-rev-完整步骤",
          "items": [
            {
              "text": "5.1 下载",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-1-下载"
            },
            {
              "text": "5.2 安装",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-2-安装"
            },
            {
              "text": "5.3 导入订阅",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-3-导入订阅"
            },
            {
              "text": "5.4 选择节点和策略",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-4-选择节点和策略"
            },
            {
              "text": "5.5 开启系统代理",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-5-开启系统代理"
            },
            {
              "text": "5.6 macOS 开启 TUN 模式",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-6-macos-开启-tun-模式"
            },
            {
              "text": "5.7 macOS 关闭与恢复",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_5-7-macos-关闭与恢复"
            }
          ]
        },
        {
          "text": "6. Windows：Clash Verge Rev 完整步骤",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-windows-clash-verge-rev-完整步骤",
          "items": [
            {
              "text": "6.1 下载与安装",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-1-下载与安装"
            },
            {
              "text": "6.2 导入订阅",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-2-导入订阅"
            },
            {
              "text": "6.3 选择节点",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-3-选择节点"
            },
            {
              "text": "6.4 开启系统代理",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-4-开启系统代理"
            },
            {
              "text": "6.5 Windows 开启 TUN 模式",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-5-windows-开启-tun-模式"
            },
            {
              "text": "6.6 Windows 常见恢复命令",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_6-6-windows-常见恢复命令"
            }
          ]
        },
        {
          "text": "7. Android：FlClash / Clash Meta for Android",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-android-flclash-clash-meta-for-android",
          "items": [
            {
              "text": "7.1 客户端选择",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-1-客户端选择"
            },
            {
              "text": "7.2 安装 APK",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-2-安装-apk"
            },
            {
              "text": "7.3 导入订阅",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-3-导入订阅"
            },
            {
              "text": "7.4 启动 VPN",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-4-启动-vpn"
            },
            {
              "text": "7.5 选择节点",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-5-选择节点"
            },
            {
              "text": "7.6 Android 注意事项",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_7-6-android-注意事项"
            }
          ]
        },
        {
          "text": "8. iOS：Stash / Shadowrocket",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-ios-stash-shadowrocket",
          "items": [
            {
              "text": "8.1 客户端选择",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-1-客户端选择"
            },
            {
              "text": "8.2 Stash 导入订阅",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-2-stash-导入订阅"
            },
            {
              "text": "8.3 Stash 选择节点",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-3-stash-选择节点"
            },
            {
              "text": "8.4 Shadowrocket 导入订阅",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-4-shadowrocket-导入订阅"
            },
            {
              "text": "8.5 iOS 注意事项",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_8-5-ios-注意事项"
            }
          ]
        },
        {
          "text": "9. 模式怎么选",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_9-模式怎么选",
          "items": [
            {
              "text": "9.1 日常默认",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_9-1-日常默认"
            },
            {
              "text": "9.2 临时测试",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_9-2-临时测试"
            },
            {
              "text": "9.3 恢复网络",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_9-3-恢复网络"
            },
            {
              "text": "9.4 系统代理 vs TUN",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_9-4-系统代理-vs-tun"
            }
          ]
        },
        {
          "text": "10. 验证是否成功",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_10-验证是否成功",
          "items": [
            {
              "text": "10.1 最简单验证",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_10-1-最简单验证"
            },
            {
              "text": "10.2 看日志",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_10-2-看日志"
            },
            {
              "text": "10.3 看 IP 和 DNS",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_10-3-看-ip-和-dns"
            }
          ]
        },
        {
          "text": "11. 常见问题排查",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-常见问题排查",
          "items": [
            {
              "text": "11.1 订阅导入失败",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-1-订阅导入失败"
            },
            {
              "text": "11.2 有节点但无法连接",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-2-有节点但无法连接"
            },
            {
              "text": "11.3 浏览器能用，其他软件不能用",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-3-浏览器能用-其他软件不能用"
            },
            {
              "text": "11.4 开启后全部断网",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-4-开启后全部断网"
            },
            {
              "text": "11.5 国内网站变慢",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-5-国内网站变慢"
            },
            {
              "text": "11.6 政务、网银、律所 OA 异常",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_11-6-政务、网银、律所-oa-异常"
            }
          ]
        },
        {
          "text": "12. 律师工作特别注意事项",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_12-律师工作特别注意事项",
          "items": [
            {
              "text": "12.1 保密",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_12-1-保密"
            },
            {
              "text": "12.2 日志",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_12-2-日志"
            },
            {
              "text": "12.3 证书与 MITM",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_12-3-证书与-mitm"
            },
            {
              "text": "12.4 网页证据保存",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_12-4-网页证据保存"
            }
          ]
        },
        {
          "text": "13. 日常使用推荐流程",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_13-日常使用推荐流程",
          "items": [
            {
              "text": "13.1 电脑端",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_13-1-电脑端"
            },
            {
              "text": "13.2 手机端",
              "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_13-2-手机端"
            }
          ]
        },
        {
          "text": "14. 推荐配置原则",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_14-推荐配置原则"
        },
        {
          "text": "15. 备份与迁移",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_15-备份与迁移"
        },
        {
          "text": "16. 成熟参考资料",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V1#_16-成熟参考资料"
        }
      ]
    }
  ],
  "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V2": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "梯子使用全教程 HTML 自包含版",
      "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V2"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "内容索引",
          "link": "/tutorials/proxy-clash-verge-full-guide-ABL-20260707-V2#内容索引"
        }
      ]
    }
  ],
  "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1": [
    {
      "text": "返回栏目列表",
      "link": "/tutorials/"
    },
    {
      "text": "Windows + Codex 法律工作流环境安装教程",
      "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1"
    },
    {
      "text": "本页目录",
      "items": [
        {
          "text": "1. 任务理解与安装策略",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_1-任务理解与安装策略"
        },
        {
          "text": "2. 先确认 Windows 基础环境",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_2-先确认-windows-基础环境"
        },
        {
          "text": "3. 安装 Codex App",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_3-安装-codex-app"
        },
        {
          "text": "4. 安装基础命令行工具",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_4-安装基础命令行工具"
        },
        {
          "text": "5. 安装浏览器与办公组件",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_5-安装浏览器与办公组件"
        },
        {
          "text": "6. 安装 PDF、图片与 OCR 工具",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_6-安装-pdf、图片与-ocr-工具",
          "items": [
            {
              "text": "6.1 原生 Windows 工具",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_6-1-原生-windows-工具"
            },
            {
              "text": "6.2 安装 Tesseract 中文语言包",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_6-2-安装-tesseract-中文语言包"
            }
          ]
        },
        {
          "text": "7. 可选：安装 WSL2 作为 OCR/PDF 增强环境",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_7-可选-安装-wsl2-作为-ocr-pdf-增强环境"
        },
        {
          "text": "8. 建立 Python 法律工作虚拟环境",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_8-建立-python-法律工作虚拟环境"
        },
        {
          "text": "9. 法律文档处理推荐验证链条",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_9-法律文档处理推荐验证链条",
          "items": [
            {
              "text": "9.1 DOCX 合同审查",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_9-1-docx-合同审查"
            },
            {
              "text": "9.2 DOCX 转 PDF 检查页数和签署页",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_9-2-docx-转-pdf-检查页数和签署页"
            },
            {
              "text": "9.3 PDF 底稿验证",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_9-3-pdf-底稿验证"
            }
          ]
        },
        {
          "text": "10. Codex 全局法律工作配置",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_10-codex-全局法律工作配置",
          "items": [
            {
              "text": "10.1 创建全局 AGENTS.md",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_10-1-创建全局-agents-md"
            },
            {
              "text": "10.2 项目级 AGENTS.md",
              "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_10-2-项目级-agents-md"
            }
          ]
        },
        {
          "text": "11. 建议安装或保留的法律工作 Skills",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_11-建议安装或保留的法律工作-skills"
        },
        {
          "text": "12. MCP 与外部数据源",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_12-mcp-与外部数据源"
        },
        {
          "text": "13. 浏览器证据 PDF 规则",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_13-浏览器证据-pdf-规则"
        },
        {
          "text": "14. 一键安装脚本（PowerShell）",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_14-一键安装脚本-powershell"
        },
        {
          "text": "15. 完整验收清单",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_15-完整验收清单"
        },
        {
          "text": "16. 推荐目录结构",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_16-推荐目录结构"
        },
        {
          "text": "17. 什么时候用 PowerShell，什么时候用 WSL",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_17-什么时候用-powershell-什么时候用-wsl"
        },
        {
          "text": "18. 官方参考来源",
          "link": "/tutorials/windows-codex-legal-workflow-setup-ABL-20260707-V1#_18-官方参考来源"
        }
      ]
    }
  ],
  "/tutorials/": [
    {
      "text": "AI 教程",
      "link": "/tutorials/"
    }
  ],
  "/latest/": [
    {
      "text": "最新文章",
      "link": "/latest/"
    }
  ],
  "/tools/": [
    {
      "text": "实用工具",
      "link": "/tools/"
    },
    {
      "text": "法律工具",
      "link": "/tools/legal-tools"
    },
    {
      "text": "网核网站",
      "link": "/tools/network-check-sites"
    }
  ]
}
