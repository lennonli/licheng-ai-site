<template>
  <section class="home-popular" aria-labelledby="home-popular-title">
    <div class="home-popular-heading">
      ...
      <span class="home-popular-period">近 30 天（可查询期间） · 每日更新</span>
    </div>
...
onMounted(async () => {
  try {
    const [analyticsResponse, indexResponse] = await Promise.all([
      fetch('/api/site-analytics?public=top', { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(8000) }),
      fetch('/content-index.json', { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(8000) })
    ])
