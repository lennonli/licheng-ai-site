...
const titleOverrides = new Map([
  ...
  ['agents/私募基金投资业务协议审阅全局指令.md', '私募基金投资业务协议审阅全局指令'],
  ['agents/私募基金老股东不参与投资审核提示词.md', '私募基金老股东不参与投资审核提示词'],
  ['agents/私募基金新股东投资文件审核提示词.md', '私募基金新股东投资文件审核提示词'],
  ['agents/蒸馏投资机构内部投资文件提示词.md', '蒸馏投资机构内部投资文件提示词'],
  ...
  ['skills/ipo-kb', '证券业务法律问询案例和证券法规检索skill'],
...
const summaryOverrides = new Map([
  ...
  ['agents/私募基金投资业务协议审阅全局指令.md', '私募基金投资交易文件 AI 审阅总纲（V2）：四种审阅模式与兜底规则、十二项最低必查项、合规底线、风险分级与数量校准、复核过滤规则和 Word 审查表输出规范，与三篇场景提示词配套使用。'],
  ['agents/私募基金老股东不参与投资审核提示词.md', '私募基金作为既有老股东、本轮融资不参与新增投资时的交易文件审阅提示词：以前一轮已签署文件为唯一基准识别既有权益减损，禁止站在新投资人立场重新审阅。'],
  ['agents/私募基金新股东投资文件审核提示词.md', '投资机构作为新股东、以己方模板为基础谈判时的投资文件审核提示词：只审对方修改是否削弱或架空投资方权利，区分正常谈判调整与实质减损。'],
  ['agents/蒸馏投资机构内部投资文件提示词.md', '从投资机构历史投资协议、股东协议等模板反向蒸馏《机构标准条款及审阅规则库》，供 AI 后续按机构口径判断待审协议符合、等效或偏离。'],
  ...
  ['skills/ipo-kb', '证券业务审核问询案例 12 库（IPO/再融资/并购重组四年度，2,585 家）＋投行法规库＋补充法律意见书库，共 14 个知识库；MCP 语义检索优先、本地克隆直查兜底，全库网页在线阅读。'],
...
function writeGeneratedSidebar() {
  ...
  const sidebar = {
    ...
    ...buildSectionSidebar({
      section: 'refi2024',
      indexText: '再融资案例库·2024',
      destDir: refi2024Dest
    }),
    ...buildSectionSidebar({
      section: 'refi2023',
      indexText: '再融资案例库·2023',
...
<section class="home-grid" aria-label="内容入口">
  ...
  <a class="home-card" href="/refi2024/">
    <span class="home-card-index">14 / Refi Cases 2024</span>
...
function aiTutorialSection(base, repoUrl, exampleCase) {
  ...
  const isRefi = base.startsWith('/refi')
...
function buildKbYear({ key, base, title, lead, entries, annualFile, annualTitle, sourceDir = null }) {
  ...
  if (key === 'kb2023' || key.startsWith('refi')) {
    for (const name of readDirSafe(dest).filter((entry) => entry.endsWith('.md'))) {
      const file = path.join(dest, name)
      const markdown = readFileSync(file, 'utf8')
  ...
  for (const board of [...kbBoardOrder, '其他']) {
    ...
    indexMd += indexCardList(rows.map((entry) => {
      ...
      const summary = key.startsWith('refi')
...
// 2024 年再融资年度库（/refi2024/）
const refi2024Src = path.join(cacheDir, 'kbmono', 'refi2024')
const refi2024IndexPath = path.join(refi2024Src, 'scripts', 'index.json')
const refi2024Entries = existsSync(refi2024IndexPath) ? JSON.parse(readFileSync(refi2024IndexPath, 'utf8')) : []
const refi2024Dest = buildKbYear({
  key: 'refi2024',
  base: '/refi2024',
  title: '再融资审核案例库 · 2024年度',
  lead: '2024 年度深市再融资（定向增发/可转债）审核问询案例 33 家（24 家注册生效、8 家撤回、1 家被上市委否决后重新申报获准），一案一文，沉淀募集资金用途、财务性投资、前次募集资金、发行对象与定价等问询要点与律师核查结论。可用站内搜索按公司简称、代码或法律问题关键词检索。',
  entries: refi2024Entries,
  annualFile: '2024年度总结.md',
  annualTitle: '📊 2024 年度总结报告',
  sourceDir: refi2024Src
})

// 2023 年再融资年度库（/refi2023/）
const refi2023Src = path.join(cacheDir, 'kbmono', 'refi2023')
const refi2023IndexPath = path.join(refi2023Src, 'scripts', 'index.json')
const refi2023Entries = existsSync(refi2023IndexPath) ? JSON.parse(readFileSync(refi2023IndexPath, 'utf8')) : []
const refi2023Dest = buildKbYear({
  key: 'refi2023',
  base: '/refi2023',
  title: '再融资审核案例库 · 2023年度',
  lead: '2023 年度沪深再融资（定向增发/可转债/配股）审核问询案例 390 家（313 家注册生效、68 家终止、9 家审核中），全面注册制首年与 827 新规撤回潮的全景样本，一案一文，沉淀募集资金用途、财务性投资、前次募集资金、发行对象与定价等问询要点与律师核查结论。可用站内搜索按公司简称、代码或法律问题关键词检索。',
  entries: refi2023Entries,
  annualFile: '2023年度总结.md',
  annualTitle: '📊 2023 年度总结报告',
